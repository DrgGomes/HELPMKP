...
try {
  ...
  setPdvModal(null); setPdvQtd(1); setPdvValorFinal('');
} catch (err) {
  console.error(err);
...
{idEdicao ? 'Edição de Produto' : 'Engenharia de Produto'}
Cadastre insumos, defina a embalagem e a meta para alimentar o algoritmo de inteligência.
    ...
    <form onSubmit={lidarSalvarProduto} className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4 bg-slate-50 p-6 rounded-2xl border border-slate-100">
          ...
          <div className="flex gap-4">
            <div className="w-1/2"><p className="text-xs font-bold text-slate-500 mb-1">Estoque</p><input type="number" required placeholder="0" value={estoque} onChange={e => setEstoque(e.target.value)} className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl font-black text-blue-600" /></div>
            <div className="w-1/2"><p className="text-xs font-bold text-slate-500 mb-1">Alerta Min.</p><input type="number" required placeholder="Min" value={estoqueMinimo} onChange={e => setEstoqueMinimo(e.target.value)} className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl font-black text-rose-500" /></div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900 p-8 rounded-2xl shadow-xl flex flex-col lg:flex-row items-center gap-6 relative overflow-hidden">
