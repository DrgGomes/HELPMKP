...
export default function Fornecedores({ fornecedores, produtos, compras }: { fornecedores: Fornecedor[], produtos: Produto[], compras: Compra[] }) {
  ...
  const adicionarAoCarrinho = () => {
    ...
    const novoItem: ItemCompra = { produtoId: prod.id, nome: prod.titulo, quantidade: quantidadeDesejada, custoUnitario: custo, subtotal: quantidadeDesejada * custo };
    setItensCarrinho([...itensCarrinho, novoItem]);
    setProdutoSelecionado(''); setQuantidadeDesejada(1); setCustoUnitario('');
  };

  const removerDoCarrinho = (index: number) => {
    const novos = [...itensCarrinho]; novos.splice(index, 1); setItensCarrinho(novos);
  };
  ...
  const finalizarOrdem = async () => {
    ...
    try {
      ...
      console.error(e); alert("Falha ao gerar ordem.");
    }
  ...
  const registrarRecebimento = async (compra: Compra) => {
    ...
    try {
      // 1. Atualizar compra
      const { error: errComp } = await supabase.from('compras').update({ status_chegada: 'recebido', fatura_gerada: true }).eq('id', compra.id).eq('user_id', userId);
      ...
      for (const item of compra.itens) {
        ...
        if (prodExistente) {
          ...
          const { error: errProd } = await supabase.from('produtos').update({ estoque: novoEstoque }).eq('id', prodExistente.id).eq('user_id', userId);
          if (errProd) console.warn('Erro ao atualizar estoque do produto:', errProd.message);
        }
      }
      ...
      const { error: errLanc } = await supabase.from('lancamentos').insert({
        ...
        data_vencimento: compra.dataPagamento || compra.dataCompra || new Date().toISOString().split('T')[0],
        data_lancamento: compra.dataCompra || new Date().toISOString().split('T')[0],
        status: 'pendente',
        categoria: 'Compras de Estoque',
        fornecedor_id: compra.fornecedorId,
        compra_id: compra.id,
      });
      ...
      console.error(e); alert("Erro ao processar recebimento.");
    }
  ...
  const corrigirFaturasAntigas = async () => {
    ...
    setProcessandoRecebimento(true);
    try {
      for (const compra of comprasSemFatura) {
        const { error: errComp } = await supabase.from('compras').update({ fatura_gerada: true }).eq('id', compra.id).eq('user_id', userId);
        if (errComp) console.warn('Erro ao atualizar compra:', errComp.message);
        ...
        if (errLanc) console.warn('Erro ao criar lançamento:', errLanc.message);
      }
  ...
  const salvarFornecedor = async (e: React.FormEvent) => {
    ...
    if (!nomeForn) return;
    const userId = await getUserId(); if (!userId) return;
    const dados = { nome: nomeForn, contato: contatoForn, categoria_insumo: categoriaForn };
    try {
      if (idFornecedorEdicao) {
        const { error } = await supabase.from('fornecedores').update(dados).eq('id', idFornecedorEdicao).eq('user_id', userId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('fornecedores').insert({ ...dados, user_id: userId });
        if (error) throw error;
      }
      setIdFornecedorEdicao(null); setNomeForn(''); setContatoForn(''); setCategoriaForn('');
    } catch (error) { console.error(error); alert('Erro ao salvar fornecedor.'); }
  };
  ...
  const executarImpressaoProfissional = () => { setTimeout(() => window.print(), 800); };
  ...
  return (
    <div className="animate-fade-in max-w-[1600px] mx-auto space-y-6 sm:space-y-8 no-print">
      <style dangerouslySetInnerHTML={{__html: `
        ...
        @media print { #root { display: none !important; } body { background: white !important; margin: 0 !important; padding: 0 !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; } .print-portal { display: block !important; width: 100% !important; color: black !important; } @page { margin: 10mm; size: auto; } }
      `}} />
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-6">
        <div>
          <h2 className="text-3xl sm:text-4xl font-black text-slate-800 tracking-tight flex items-center gap-3"><span>🚚</span> Cargas</h2>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start animate-fade-in">
          <div className="xl:col-span-5 bg-white p-5 sm:p-8 rounded-3xl shadow-sm border border-slate-200">
            ...
            <div className="space-y-4">
              <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Fábrica</label><select value={fornecedorSelecionado} onChange={(e) => setFornecedorSelecionado(e.target.value)} className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-indigo-500"><option value="">Selecionar Fábrica...</option>{fornecedores.map(f => <option key={f.id} value={f.id}>{f.nome}</option>)}</select></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Emissão</label><input type="date" value={dataEmissao} onChange={(e) => setDataEmissao(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none" /></div>
                <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Vencimento</label><input type="date" value={dataVencimento} onChange={(e) => setDataVencimento(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none" /></div>
              </div>
              <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Vale / NF</label><input type="text" placeholder="Ex: VALE-1234" value={numeroVale} onChange={(e) => setNumeroVale(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold font-mono text-slate-700 outline-none" /></div>
      ...
      {abaAtiva === 'receber' && (
        <div className="animate-fade-in space-y-8">
          ...
          <div className="bg-[#064e3b] rounded-[2rem] p-6 sm:p-10 text-center shadow-xl border border-emerald-900/50 max-w-4xl mx-auto">
            ...
            <div className="max-w-xl mx-auto"><input type="text" placeholder="ORD-123456" value={codigoBip} onChange={(e) => setCodigoBip(e.target.value)} onKeyDown={lidarBip} className="w-full bg-[#022c22] border-2 border-emerald-500/50 text-emerald-400 text-center text-xl sm:text-3xl font-black font-mono py-4 sm:py-5 rounded-xl outline-none placeholder:text-emerald-900/40 focus:border-emerald-400" /></div>
          </div>
          <div>
            ...
            {fornecedores.map(forn => (
              <div key={forn.id} className="bg-white p-4 sm:p-5 rounded-2xl shadow-sm border border-slate-200 flex justify-between items-center">
                ...
                <div className="flex gap-1 sm:gap-2">
                  ...
                  <button onClick={() => excluirFornecedor(forn.id)} className="w-8 h-8 sm:w-10 sm:h-10 bg-rose-50 text-rose-500 rounded-lg flex items-center justify-center">✕</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="mt-10 sm:mt-12">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-4">
          ...
          {selecionadosImpressao.length > 0 && (
            <button onClick={() => setOrdensParaImprimir(compras.filter(c => selecionadosImpressao.includes(c.id)))} className="w-full sm:w-auto px-4 sm:px-5 py-2.5 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-md flex items-center justify-center gap-2 animate-fade-in"><span>🖨️</span> Imprimir {selecionadosImpressao.length} Selecionadas</button>
          )}
        </div>
            ...
            <table className="w-full text-left text-[10px] sm:text-xs border-collapse whitespace-nowrap">
              <thead>
                <tr className="bg-slate-900 text-white font-black uppercase tracking-widest text-[8px] sm:text-[9px] border-b border-slate-800">
                  ...
                  <th className="p-3 sm:p-4">Código</th><th className="p-3 sm:p-4">Fábrica</th><th className="p-3 sm:p-4">Emissão</th><th className="p-3 sm:p-4">Status</th><th className="p-3 sm:p-4 text-right">Total</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 font-bold text-slate-700">
                {compras.map(c => (
                  <tr key={c.id} className={`hover:bg-slate-50 ${selecionadosImpressao.includes(c.id) ? 'bg-indigo-50/30' : ''}`}>
                    ...
                    <td className="p-3 sm:p-4 font-mono text-slate-900">{c.codigoOrdem}</td><td className="p-3 sm:p-4">{c.fornecedorNome}</td><td className="p-3 sm:p-4 font-mono">{c.dataCompra.split('-').reverse().join('/')}</td>
        ...
        <>
          <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-[200] flex justify-center items-center p-4 animate-fade-in print:hidden">
            <div className="bg-white w-full max-w-2xl rounded-[2rem] shadow-2xl overflow-hidden border border-slate-200 flex flex-col max-h-[90vh]">
              ...
              <div className="p-4 sm:p-8 overflow-y-auto flex-1 bg-slate-100"><div className="bg-white p-4 sm:p-8 shadow-sm border border-slate-200 rounded-xl pointer-events-none opacity-80"><p className="text-center font-bold text-slate-500 mb-6 text-[10px] sm:text-sm uppercase tracking-widest">Pré-visualização Simbólica (Clique em Imprimir)</p></div></div>
            ...
            {ordensParaImprimir.map((ordem, index) => (
              <div key={`print-${ordem.id}`} style={{ pageBreakInside: 'avoid' }} className={index !== ordensParaImprimir.length - 1 ? 'border-b border-dashed border-slate-400 pb-12 mb-12' : ''}>
                <div className="flex justify-between items-start border-b-2 border-black pb-4 mb-4">
                  <div><h1 className="text-2xl font-black tracking-tight uppercase">Ordem de Fábrica</h1><p className="text-[10px] font-bold text-slate-600 mt-1">HelpMkp Enterprise ERP</p>
                    <div className="mt-3 space-y-0.5 font-mono text-[10px] text-black"><p><strong>FABRICA:</strong> {ordem.fornecedorNome}</p><p><strong>EMISSÃO:</strong> {ordem.dataCompra.split('-').reverse().join('/')}</p><p><strong>VENCIMENTO:</strong> {ordem.dataPagamento?.split('-').reverse().join('/') || '---'}</p>{ordem.numeroVale && <p><strong>VALE:</strong> {ordem.numeroVale}</p>}</div>
                  </div>
                  <div className="text-center"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${ordem.codigoOrdem}`} alt="QR" loading="eager" className="w-16 h-16 mx-auto" /><img src={`https://barcode.tec-it.com/barcode.ashx?data=${ordem.codigoOrdem}&code=Code128`} alt="Barcode" loading="eager" className="h-8 w-auto object-contain mx-auto mt-2" /></div>
                </div>
                <table className="w-full text-left text-[10px] border-collapse mb-4 text-black">
                  <thead><tr className="border-b border-black font-black uppercase"><th className="p-1">Insumo</th><th className="p-1 text-center">Qtd</th><th className="p-1 text-right">Unidade</th><th className="p-1 text-right">Subtotal</th></tr></thead>
                  <tbody>{ordem.itens.map((item, idx) => (<tr key={idx} className="border-b border-slate-200"><td className="p-1 font-bold">{item.nome}</td><td className="p-1 text-center font-mono">{item.quantidade}</td><td className="p-1 text-right font-mono">R$ {item.custoUnitario.toFixed(2)}</td><td className="p-1 text-right font-mono font-bold">R$ {item.subtotal.toFixed(2)}</td></tr>))}</tbody>
                </table>
                <div className="border-t border-black pt-2 flex justify-between items-center text-black"><div className="text-[10px]"><p>Status: {ordem.statusChegada.toUpperCase()}</p></div><div className="text-right"><p className="text-[8px] font-black uppercase tracking-widest mb-0.5">Total</p><p className="text-xl font-black font-mono">R$ {ordem.valorTotal.toFixed(2)}</p></div></div>
              </div>
            ))}
          </div>
        </>, document.body
