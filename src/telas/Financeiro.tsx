...
export default function Financeiro({ lancamentos, compras, fornecedores }: FinanceiroProps) {
  ...
  // NOVOS ESTADOS: Controle de Recorrência (Multiplicação)
  ...
  // --- MÁGICA: SALVAR E MULTIPLICAR LANÇAMENTOS ---
    ...
    try {
      if (idEdicao || !isRecorrente) {
        ...
        for (let i = 0; i < qtdMeses; i++) {
          ...
          const objLanc = new Date(dataLancamento + 'T12:00:00');
          objLanc.setMonth(objLanc.getMonth() + i);

          await addDoc(collection(db, 'usuarios', userId, 'lancamentos'), {
            tipo,
            descricao: qtdMeses > 1 ? `${descricao} (${i + 1}/${qtdMeses})` : descricao,
            valor: valorNum,
            dataVencimento: objVenc.toISOString().split('T')[0],
            dataLancamento: objLanc.toISOString().split('T')[0],
            categoria,
            status: 'pendente',
            fornecedorId: fId,
            recorrente: true,
  ...
  // --- INTELIGÊNCIA: APAGAR SÉRIE EM MASSA OU APENAS O MÊS ---
  const excluirLancamento = async (lanc: LancamentoFinanceiro) => {
    ...
    if (lanc.grupoRecorrenciaId) {
      ...
      if (resposta?.toUpperCase() === 'APENAS') {
        await deleteDoc(doc(db, 'usuarios', userId, 'lancamentos', lanc.id));
        ...
        alert("Toda a série recorrente foi eliminada do caixa.");
      }
    } else {
  ...
  return (
    <div className="animate-fade-in max-w-7xl mx-auto space-y-6 relative">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 no-print">
        ...
        <button onClick={() => setMostrarRelatorio(!mostrarRelatorio)} className={`px-5 py-3 rounded-xl font-bold flex items-center gap-2 border transition-all ${mostrarRelatorio ? 'bg-amber-100 text-amber-700 border-amber-300' : 'bg-white text-slate-700 border-slate-300 shadow-sm'}`}>🔍 Buscar & Relatórios</button>
      </header>
      ...
      {mostrarRelatorio && (
        <div className="bg-white p-6 rounded-2xl border border-slate-300 shadow-lg space-y-5 no-print animate-fade-in">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-3 text-xs">
            ...
            <div><label className="block font-bold text-slate-500 uppercase mb-1">Mês Vencimento</label><select value={draftMes} onChange={(e) => setDraftMes(Number(e.target.value))} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg font-bold text-slate-700 outline-none"><option value={0}>Todos</option>{Array.from({ length: 12 }, (_, i) => (<option key={i+1} value={i+1}>{new Date(0, i).toLocaleString('pt-BR', { month: 'short' })}</option>))}</select></div>
            <div><label className="block font-bold text-slate-500 uppercase mb-1">Ano</label><select value={draftAno} onChange={(e) => setDraftAno(Number(e.target.value))} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg font-bold text-slate-700 outline-none"><option value={0}>Todos</option><option value={2025}>2025</option><option value={2026}>2026</option><option value={2027}>2027</option></select></div>
            ...
            <div><label className="block font-bold text-slate-500 uppercase mb-1">Fornecedor</label><select value={draftFornecedor} onChange={(e) => setDraftFornecedor(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg font-bold text-slate-700 outline-none truncate"><option value="todos">Qualquer Fornecedor</option>{fornecedores.map(f => <option key={f.id} value={f.id}>{f.nome}</option>)}</select></div>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start animate-fade-in print:hidden">
            <div className="xl:col-span-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-fit sticky top-6">
              ...
              <form onSubmit={lidarSalvar} className="space-y-4">
                ...
                {/* INTERFACE NOVA: DISPARADOR DE DUPLICIDADE EM MASSA */}
                {!idEdicao && (
                  <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200 space-y-2">
                    ...
                    {isRecorrente && (
                      <div className="animate-fade-in">
                        <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Repetir por quantos meses?</label>
                        <input type="number" min="2" max="36" value={mesesRepetir} onChange={(e) => setMesesRepetir(e.target.value)} className="w-full px-3 py-2 border rounded-xl font-black text-sm text-blue-600" />
                      </div>
                    )}
                  </div>
                )}
