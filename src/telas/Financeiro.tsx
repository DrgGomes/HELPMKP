...
export default function Financeiro({ lancamentos, compras, fornecedores }: FinanceiroProps) {
  ...
  const lidarSalvar = async (e: React.FormEvent) => {
    ...
    const dados: any = { tipo, descricao, valor: parseFloat(valor), dataVencimento, categoria };
  ...
  const iniciarEdicao = (lanc: LancamentoFinanceiro) => {
    ...
    setValor(lanc.valor.toString()); setDataVencimento(lanc.dataVencimento);
  ...
  return (
    <div className="animate-fade-in max-w-7xl mx-auto space-y-6 relative">
      ...
      {/* Estilos para o PDF do Relatório */}
      ...
      {/* --- MÓDULO DE RELATÓRIO E FILTROS AVANÇADOS --- */}
      {mostrarRelatorio && (
        <div className="bg-white p-6 rounded-2xl border border-slate-300 shadow-lg space-y-5 no-print animate-fade-in">
          <div className="border-b border-slate-200 pb-3 flex justify-between items-center">
            <h3 className="font-black text-slate-800">🛠️ Extrator de Relatórios</h3>
            <button onClick={() => window.print()} className="bg-slate-900 hover:bg-slate-800 text-white font-black px-6 py-2.5 rounded-lg flex items-center gap-2 shadow-lg">
              🖨️ Gerar PDF Destes Dados
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 text-xs">
            ...
            <div>
              ...
              <select value={tipoFiltro} onChange={(e) => setTipoFiltro(e.target.value as any)} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg font-bold text-slate-700 outline-none"><option value="todos">Todos</option><option value="despesa">Despesas (-)</option><option value="receita">Receitas (+)</option></select>
            </div>
            <div>
      ...
      <div id="relatorio-financeiro-pdf">
        ...
        <div className="hidden print:block mb-8 border-b-2 border-slate-800 pb-4">
          ...
          <p className="text-xs text-slate-500 mt-1">
            Filtros Aplicados: Mês {mesFiltro === 0 ? 'Todos' : mesFiltro}/{anoFiltro === 0 ? 'Todos' : anoFiltro} | 
            Status: {statusFiltro.toUpperCase()} | 
            Fornecedor: {fornecedorFiltro === 'todos' ? 'TODOS' : (fornecedores.find(f=>f.id === fornecedorFiltro)?.nome || 'AVULSOS')}
          </p>
        </div>

        {/* RESUMO DO PDF (E Visível na Tela de Extrato se o filtro estiver aberto) */}
        {(mostrarRelatorio || document.getElementById('relatorio-financeiro-pdf')) && (
          <div className="grid grid-cols-3 gap-4 mb-6 print:grid">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-center">
              ...
              <p className="text-xl font-black text-emerald-600">R$ {resumoFiltrado.receitas.toFixed(2)}</p>
            </div>
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-center">
              <p className="text-[10px] font-bold text-slate-500 uppercase">Despesas (-)</p>
              <p className="text-xl font-black text-rose-600">R$ {resumoFiltrado.despesas.toFixed(2)}</p>
            </div>
            <div className="bg-slate-800 p-4 rounded-xl border border-slate-900 text-center text-white">
              <p className="text-[10px] font-bold text-slate-400 uppercase">Saldo do Relatório</p>
              <p className={`text-xl font-black ${resumoFiltrado.saldo >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>R$ {resumoFiltrado.saldo.toFixed(2)}</p>
            </div>
          </div>
        )}
        ...
        <div className="hidden print:block w-full">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-400 bg-slate-100 font-bold uppercase">
                <th className="p-2">Vencimento</th>
                <th className="p-2">Descrição / Fornecedor</th>
            ...
            <tbody className="divide-y divide-slate-300">
              {lancamentosFiltrados.map(l => (
                <tr key={l.id}>
                  ...
                  <td className="p-2">
                    <span className="font-bold">{l.descricao}</span>
                    {l.fornecedorId && <div className="text-[10px] text-slate-500">{fornecedores.find(f=>f.id === l.fornecedorId)?.nome}</div>}
                  </td>
                  <td className="p-2">{l.categoria}</td>
                  <td className="p-2 text-center font-bold uppercase">{l.status}</td>
                  <td className={`p-2 text-right font-black ${l.tipo === 'despesa' ? 'text-rose-600' : 'text-emerald-600'}`}>
            ...
            <div className="xl:col-span-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-fit sticky top-6">
              ...
              <form onSubmit={lidarSalvar} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" placeholder="Ex: Fixo, Variável, Imposto" value={categoria} onChange={(e) => setCategoria(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium outline-none" />
                </div>

                {tipo === 'despesa' && (
                  <div className="bg-rose-50 p-3 rounded-xl border border-rose-100">
                    <label className="block text-[10px] font-bold text-rose-500 uppercase mb-1">Vincular a Fornecedor (Opcional)</label>
                    <select value={fornSelecionado} onChange={(e) => setFornSelecionado(e.target.value)} className="w-full px-3 py-2.5 bg-white border border-rose-200 rounded-lg text-sm font-bold text-slate-700 outline-none truncate">
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  {idEdicao && <button type="button" onClick={limparFormulario} className="px-5 bg-slate-200 text-slate-600 font-bold rounded-xl hover:bg-slate-300">Cancelar</button>}
                </div>
              </form>
            </div>
        ...
        {/* --- TELA NO APP: GESTÃO DE FORNECEDORES (COM O NOVO TOTALIZADOR EMBAIXO) --- */}
        {abaAtiva === 'fornecedores' && (
          <div className="space-y-6 animate-fade-in print:hidden pb-32">
            
            {/* O NOVO GRANDE TOTALIZADOR DE DÍVIDAS */}
            <div className="bg-rose-900 p-8 rounded-3xl shadow-xl border border-rose-800 text-white flex flex-col md:flex-row justify-between items-center gap-6">
              <div>
                <h3 className="text-xl font-bold text-rose-200 uppercase tracking-widest mb-1">Risco Total em Fornecedores</h3>
                <p className="text-sm text-rose-300">Soma de todas as faturas pendentes de material da fábrica.</p>
              </div>
              <div className="text-5xl font-black tracking-tight text-white bg-rose-950/50 px-6 py-4 rounded-2xl border border-rose-800/50">
                R$ {TOTAL_GERAL_DEVIDO.toFixed(2)}
              </div>
            </div>
                        ...
                        return (
                          <div key={fat.id} className={`p-4 rounded-xl border flex flex-col xl:flex-row justify-between xl:items-center gap-4 ${isAtrasado ? 'bg-rose-50 border-rose-200' : 'bg-white border-slate-200 shadow-sm'}`}>
                            ...
                            <div className="flex flex-wrap md:flex-nowrap items-center gap-3">
                              ...
                              <button onClick={() => alternarStatus(fat)} className="px-4 py-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 border border-emerald-300 rounded-lg text-xs font-black uppercase transition-colors">Pagar Agora</button>
                            </div>
                          </div>
                        );
                      })}
