...
export default function Financeiro({ lancamentos, compras, fornecedores }: FinanceiroProps) {
  ...
  const aplicarFiltros = () => {
    setBuscaDescricao(draftBusca);
    setMesFiltro(draftMes);
    setAnoFiltro(draftAno);
    setStatusFiltro(draftStatus);
    setTipoFiltro(draftTipo);
    setFornecedorFiltro(draftFornecedor);
  };

  const limparFiltros = () => {
    setDraftBusca(''); setBuscaDescricao('');
    setDraftMes(0); setMesFiltro(0);
    setDraftAno(0); setAnoFiltro(0);
    setDraftStatus('todos'); setStatusFiltro('todos');
    setDraftTipo('todos'); setTipoFiltro('todos');
    setDraftFornecedor('todos'); setFornecedorFiltro('todos');
  };
  ...
  const iniciarEdicao = (lanc: LancamentoFinanceiro) => {
    ...
    setDataLancamento(lanc.dataLancamento || lanc.dataVencimento);
  ...
  const excluirLancamento = async (id: string) => {
    ...
    if (userId && window.confirm("Excluir este lançamento financeiro permanentemente?")) {
  ...
  // --- INTELIGÊNCIA: FILTRAGEM AVANÇADA DO CAIXA GERAL ---
    ...
    <div className="animate-fade-in max-w-7xl mx-auto space-y-6 relative">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 no-print">
        ...
        <div className="flex gap-2">
          <button onClick={() => setMostrarRelatorio(!mostrarRelatorio)} className={`px-5 py-3 rounded-xl font-bold flex items-center gap-2 border transition-all ${mostrarRelatorio ? 'bg-amber-100 text-amber-700 border-amber-300' : 'bg-white text-slate-700 border-slate-300 shadow-sm'}`}>
            🔍 Buscar & Relatórios
          </button>
        </div>
      </header>
      ...
      {mostrarRelatorio && (
        <div className="bg-white p-6 rounded-2xl border border-slate-300 shadow-lg space-y-5 no-print animate-fade-in">
          <div className="border-b border-slate-200 pb-3">
            <h3 className="font-black text-slate-800">🔍 Painel de Busca e Filtros</h3>
            <p className="text-xs text-slate-500 mt-1">Configure os filtros abaixo e clique em Buscar para atualizar os dados na tela.</p>
          </div>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-3 text-xs">
            <div className="lg:col-span-2">
              <label className="block font-bold text-slate-500 uppercase mb-1">Buscar Palavra</label>
              <input type="text" placeholder="Ex: Energia, Compra..." value={draftBusca} onChange={(e) => setDraftBusca(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg font-bold outline-none" />
            </div>
            <div>
              <label className="block font-bold text-slate-500 uppercase mb-1">Mês Vencimento</label>
              <select value={draftMes} onChange={(e) => setDraftMes(Number(e.target.value))} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg font-bold text-slate-700 outline-none"><option value={0}>Todos</option>{Array.from({ length: 12 }, (_, i) => (<option key={i+1} value={i+1}>{new Date(0, i).toLocaleString('pt-BR', { month: 'short' })}</option>))}</select>
            </div>
            <div>
              <label className="block font-bold text-slate-500 uppercase mb-1">Ano</label>
              <select value={draftAno} onChange={(e) => setDraftAno(Number(e.target.value))} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg font-bold text-slate-700 outline-none"><option value={0}>Todos</option><option value={2025}>2025</option><option value={2026}>2026</option><option value={2027}>2027</option></select>
            </div>
            <div>
              <label className="block font-bold text-slate-500 uppercase mb-1">Status</label>
              <select value={draftStatus} onChange={(e) => setDraftStatus(e.target.value as any)} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg font-bold text-slate-700 outline-none"><option value="todos">Todos</option><option value="pendente">Pendentes</option><option value="pago">Pagos</option></select>
            </div>
            <div>
              <label className="block font-bold text-slate-500 uppercase mb-1">Tipo</label>
              <select value={draftTipo} onChange={(e) => setDraftTipo(e.target.value as any)} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg font-bold text-slate-700 outline-none"><option value="todos">Todos</option><option value="despesa">Despesas (-)</option><option value="receita">Receitas (+)</option></select>
            </div>
            <div>
              <label className="block font-bold text-slate-500 uppercase mb-1">Fornecedor</label>
              <select value={draftFornecedor} onChange={(e) => setDraftFornecedor(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg font-bold text-slate-700 outline-none truncate"><option value="todos">Qualquer Fornecedor</option><option value="">Avulsos (S/ Forn.)</option>{fornecedores.map(f => <option key={f.id} value={f.id}>{f.nome}</option>)}</select>
            </div>
          </div>

          <div className="flex flex-wrap justify-end gap-3 pt-3 border-t border-slate-100">
            <button onClick={limparFiltros} className="px-5 py-2.5 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200 transition-colors">
              Limpar Tudo
            </button>
            <button onClick={aplicarFiltros} className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-xl shadow-md transition-colors flex items-center gap-2">
              🔍 Buscar / Aplicar Filtros
            </button>
            <div className="w-px bg-slate-200 mx-2"></div>
            <button onClick={() => window.print()} className="px-6 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-black rounded-xl flex items-center gap-2 shadow-lg">
              🖨️ Exportar PDF
            </button>
          </div>
        </div>
      ...
      <div id="relatorio-financeiro-pdf" className="hidden print:block w-full">
        ...
        <table className="w-full text-left text-xs border-collapse">
          <thead><tr className="border-b border-slate-400 bg-slate-100 font-bold uppercase"><th className="p-2">Emissão</th><th className="p-2">Vencimento</th><th className="p-2">Descrição / Fornecedor</th><th className="p-2">Categoria</th><th className="p-2 text-center">Status</th><th className="p-2 text-right">Valor</th></tr></thead>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start animate-fade-in print:hidden">
          ...
          <div className="xl:col-span-8 space-y-4">
            ...
            <div className="bg-white p-2 rounded-2xl shadow-sm border border-slate-200">
              {lancamentosFiltrados.length === 0 ? <div className="p-10 text-center text-slate-400 font-bold">Nenhum lançamento encontrado com a busca atual.</div> : (
                        ...
                        <div key={fat.id} className={`p-4 rounded-xl border flex flex-col xl:flex-row justify-between xl:items-center gap-4 ${isAtrasado ? 'bg-rose-50 border-rose-200' : 'bg-white border-slate-200 shadow-sm'}`}>
                          ...
                          <div className="flex flex-wrap md:flex-nowrap items-center gap-3">
                            ...
                            <div className="flex gap-2 bg-slate-100 p-1 rounded-lg border border-slate-200">
                              <button onClick={() => adiarVencimento(fat.id, 7, fat.dataVencimento)} className="px-3 py-1.5 bg-white text-slate-600 text-[10px] font-black uppercase rounded shadow-sm hover:text-blue-600 transition-colors">+7 Dias</button>
                              <button onClick={() => adiarVencimento(fat.id, 15, fat.dataVencimento)} className="px-3 py-1.5 bg-white text-slate-600 text-[10px] font-black uppercase rounded shadow-sm hover:text-blue-600 transition-colors">+15 Dias</button>
                            </div>
                            
                            <div className="flex gap-1 border border-slate-200 rounded-lg overflow-hidden">
                              <button onClick={() => iniciarEdicao(fat)} className="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-black transition-colors" title="Editar">✏️</button>
                              <div className="w-px bg-slate-200"></div>
                              <button onClick={() => excluirLancamento(fat.id)} className="px-3 py-2 bg-rose-50 hover:bg-rose-100 text-rose-500 text-xs font-black transition-colors" title="Apagar Lançamento">🗑️</button>
                            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex justify-center items-center p-4 animate-fade-in no-print">
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="bg-slate-100 p-6 border-t border-slate-200 flex justify-between items-center mt-auto">
              <button onClick={() => excluirValeInteiro(compraModal.id)} className="px-4 py-2 text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-lg text-xs font-black transition-colors">
                🗑️ Excluir Vale Completo
              </button>
              <div className="text-right">
                <p className="font-bold text-slate-500 uppercase">Total do Vale</p>
