...
export default function Financeiro({ lancamentos, compras, fornecedores }: FinanceiroProps) {
  ...
  const excluirValeInteiro = async (compraId: string) => {
    const userId = auth.currentUser?.uid as string; if (!userId) return;
    if (window.confirm("⚠️ ATENÇÃO: Isso excluirá o Vale/Ordem de Compra e o lançamento financeiro vinculado a ele. O estoque físico NÃO será alterado automaticamente. Deseja continuar?")) {
  ...
  return (
    <div className="animate-fade-in max-w-7xl mx-auto space-y-6 relative">
      ...
      {/* --- RESUMO DO CAIXA: SÓ APARECE NA ABA EXTRATO --- */}
      {abaAtiva === 'caixa' && (
        <div className="grid grid-cols-3 gap-4 mb-6 no-print">
          ...
          <div className="bg-slate-800 p-4 rounded-xl border border-slate-900 text-center text-white"><p className="text-[10px] font-bold text-slate-400 uppercase">Saldo do Período</p><p className={`text-xl font-black ${resumoFiltrado.saldo >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>R$ {resumoFiltrado.saldo.toFixed(2)}</p></div>
        </div>
      )}

      {/* --- ESTRUTURA INVISÍVEL PARA IMPRESSÃO EM PDF --- */}
      ...
      {abaAtiva === 'caixa' && (
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start animate-fade-in print:hidden">
          ...
          <div className="xl:col-span-8 bg-white p-2 rounded-2xl shadow-sm border border-slate-200">
            {lancamentosFiltrados.length === 0 ? <div className="p-10 text-center text-slate-400 font-bold">Nenhum lançamento no mês filtrado.</div> : (
                      ...
                      return (
                        <div key={fat.id} className={`p-4 rounded-xl border flex flex-col xl:flex-row justify-between xl:items-center gap-4 ${isAtrasado ? 'bg-rose-50 border-rose-200' : 'bg-white border-slate-200 shadow-sm'}`}>
                          ...
                          <div className="flex flex-wrap md:flex-nowrap items-center gap-3">
                            ...
                            {/* BOTÕES DE ADIAR, EDITAR E EXCLUIR */}
