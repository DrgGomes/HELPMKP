...
export interface Produto { id: string; foto: string; titulo: string; categoria?: string; custoAds?: number; custoBase: number; custosAdicionais: CustoAdicional[]; custoTotal: number; tipoLucro: 'porcentagem' | 'reais'; valorLucro: number; isKit?: boolean; estoque?: number; }
...
export interface Compra { id: string; fornecedorId: string; fornecedorNome: string; dataCompra: string; itens: ItemCompra[]; valorTotal: number; statusPagamento: 'pago' | 'pendente'; }
