...
import type { Produto, Plataforma, CustoPadrao, Categoria, Fornecedor, LancamentoFinanceiro } from './types';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        setIsLogado(false); 
        setEmailUsuario('');
        setPlataformas([]); setProdutos([]); setCustosPadrao([]); setCategorias([]); setFornecedores([]); setLancamentos([]);
        unsubPlat(); unsubProd(); unsubCustos(); unsubCat(); unsubForn(); unsubLanc();
      }
      setCarregandoAuth(false);
    });

    return () => { unsubscribeAuth(); unsubPlat(); unsubProd(); unsubCustos(); unsubCat(); unsubForn(); unsubLanc(); };
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex flex-col md:flex-row font-sans text-slate-800 antialiased">
      {/* Header Mobile */}
      <div className="md:hidden bg-slate-950 text-white p-4 flex justify-between items-center z-20">
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full bg-blue-500 animate-pulse"></div>
          <h1 className="text-xl font-black text-white">HelpMkp</h1>
        </div>
        <button onClick={() => setMenuAberto(!menuAberto)} className="p-2 text-slate-400">☰</button>
      </div>
      ...
      <div className={`${menuAberto ? 'block' : 'hidden'} md:flex w-full md:w-64 bg-slate-950 text-white p-5 flex-col absolute md:relative z-10 min-h-screen md:min-h-0 border-r border-slate-900 shadow-xl overflow-y-auto`}>
        <div className="flex items-center gap-2.5 mb-9 px-3 hidden md:flex">
          ...
          <h1 className="text-2xl font-black bg-gradient-to-r from-white via-slate-200 to-slate-400 bg-clip-text text-transparent">HelpMkp</h1>
        </div>
        ...
        <nav className="space-y-1 flex-1">
          ...
          <button onClick={() => { setTelaAtiva('fornecedores'); setMenuAberto(false); }} className={`w-full text-left py-2.5 px-4 rounded-xl text-sm font-bold transition-all flex items-center gap-3 ${telaAtiva === 'fornecedores' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-900'}`}><span>🚚</span> Compras & Fornecedores</button>
        ...
        {/* Rodapé do Perfil Restaurado (Isso resolve os erros do Vercel) */}
        <div className="pt-4 border-t border-slate-900 mt-auto space-y-2">
          <button onClick={() => { setTelaAtiva('perfil'); setMenuAberto(false); }} className={`w-full p-2.5 rounded-xl transition-all flex items-center gap-3 text-left border ${telaAtiva === 'perfil' ? 'bg-slate-900 border-slate-800 text-white' : 'border-transparent text-slate-400 hover:bg-slate-900/60'}`}>
      ...
      <div className="flex-1 overflow-auto bg-slate-50/50">
        <div className="max-w-7xl mx-auto w-full p-5 md:p-8 lg:p-10">
          ...
          {telaAtiva === 'fornecedores' && <Fornecedores fornecedores={fornecedores} produtos={produtos} />}
