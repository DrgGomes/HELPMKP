...
import type { Produto, Plataforma, CustoPadrao, Categoria, Fornecedor, Compra } from './types';
...
import Fornecedores from './telas/Fornecedores'; // A TELA NOVA
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        // ESCUTANDO FORNECEDORES E COMPRAS
        ...
        unsubComp = onSnapshot(query(collection(db, 'usuarios', user.uid, 'compras'), orderBy('dataCompra', 'desc')), (snapshot) => { setCompras(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Compra))); });

      } else {
        setIsLogado(false); setEmailUsuario('');
        setPlataformas([]); setProdutos([]); setCustosPadrao([]); setCategorias([]); setFornecedores([]); setCompras([]);
        unsubPlat(); unsubProd(); unsubCustos(); unsubCat(); unsubForn(); unsubComp();
      }
      setCarregandoAuth(false);
    });

    return () => { unsubscribeAuth(); unsubPlat(); unsubProd(); unsubCustos(); unsubCat(); unsubForn(); unsubComp(); };
  }, []);

  const lidarSair = async () => { if (window.confirm("Deseja realmente sair?")) await signOut(auth); };
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex flex-col md:flex-row font-sans text-slate-800 antialiased">
      <div className="md:hidden bg-slate-950 text-white p-4 flex justify-between items-center shadow-lg border-b border-slate-900 z-20"><div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-blue-500 animate-pulse"></div><h1 className="text-xl font-black text-white">HelpMkp</h1></div><button onClick={() => setMenuAberto(!menuAberto)} className="p-2 text-slate-400">☰</button></div>
      ...
      <div className={`${menuAberto ? 'block' : 'hidden'} md:flex w-full md:w-64 bg-slate-950 text-white p-5 flex-col absolute md:relative z-10 min-h-screen md:min-h-0 border-r border-slate-900 shadow-xl overflow-y-auto`}>
        ...
        <nav className="space-y-1 flex-1">
          ...
          {/* NOVO BOTÃO DE FLUXO E FORNECEDORES */}
          <button onClick={() => { setTelaAtiva('fornecedores'); setMenuAberto(false); }} className={`w-full text-left py-2.5 px-4 rounded-xl text-sm font-bold transition-all flex items-center gap-3 ${telaAtiva === 'fornecedores' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-400 hover:bg-slate-900'}`}><span>🚚</span> Compras & Fornecedores</button>
