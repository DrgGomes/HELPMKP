...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        unsubPlat = onSnapshot(collection(db, 'usuarios', user.uid, 'plataformas'), (snapshot) => {
          setPlataformas(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Plataforma)));
        });

        unsubProd = onSnapshot(collection(db, 'usuarios', user.uid, 'produtos'), (snapshot) => {
  ...
  const lidarSair = async () => {
    if (window.confirm("Deseja realmente sair do sistema?")) await signOut(auth);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex flex-col md:flex-row font-sans text-slate-800 antialiased">
      <div className="md:hidden bg-slate-950 text-white p-4 flex justify-between items-center shadow-lg border-b border-slate-900 z-20">
        <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-blue-500 animate-pulse"></div><h1 className="text-xl font-black text-white">HelpMkp</h1></div>
